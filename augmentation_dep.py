...
class Compose(object):
    ...
    def __call__(self, img, mask, dep):
        ...
        # -------------------------------------------------
        # add depth; remove label
        assert img.size == dep.size
        for a in self.augmentations:
            img, mask, dep = a(img, mask, dep)

        # -------------------------------------------------
        # add depth; remove label
        if self.PIL2Numpy:
            img, mask, dep = np.array(img), np.array(mask, dtype=np.uint8), np.array(dep, dtype=np.float)

        return img, mask, dep

# -------------------------------------------------
# add depth and change function; remove label
class RandomCrop(object):
    ...
    def __call__(self, img, mask, dep):
        ...
        if w == tw and h == th:
            return img, mask, dep
        if w < tw or h < th:
            return (img.resize((tw, th), Image.BILINEAR), mask.resize((tw, th), Image.NEAREST), dep.resize((tw, th), Image.BILINEAR))


        x1 = random.randint(0, w - tw)
        y1 = random.randint(0, h - th)
        return (img.crop((x1, y1, x1 + tw, y1 + th)), mask.crop((x1, y1, x1 + tw, y1 + th)), dep.crop((x1, y1, x1 + tw, y1 + th)))

# -------------------------------------------------
# change label to mask
...
class RandomHorizontallyFlip(object):
    ...
    def __call__(self, img, mask, dep):
        if random.random() < self.p:
            return (img.transpose(Image.FLIP_LEFT_RIGHT), mask.transpose(Image.FLIP_LEFT_RIGHT), dep.transpose(Image.FLIP_LEFT_RIGHT))
        return img, mask, dep
...
class RandomVerticallyFlip(object):
    ...
    def __call__(self, img, mask, dep):
        if random.random() < self.p:
            return (img.transpose(Image.FLIP_TOP_BOTTOM), mask.transpose(Image.FLIP_TOP_BOTTOM), dep.transpose(Image.FLIP_TOP_BOTTOM))
        return img, mask, dep
